...
export default function MentorCard({ m, date, menteeId }) {
  ...
  async function book(slot) {
    if (!menteeId) { setMsg('Please enter your Mentee ID at the top.'); return; }
    try {
      setMsg('Booking…');
      await createBooking({
        menteeId, mentorId: m.id, slotId: slot.id,
        amount: m.rate ? Number(m.rate) * 100 : 0,
    ...
    <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm hover:shadow-md transition">
      <div className="flex items-start justify-between">
        ...
        <div className="text-right text-sm text-slate-700">
          <div><span className="font-semibold">Rate:</span> {m.rate ? `$${m.rate}` : '—'} {m.currency || ''}</div>
      ...
      <div className="mt-3">
        ...
        <div className="mt-1 flex flex-wrap gap-2">
          {m.domains?.length
            ? m.domains.map(d => (
                <span key={d} className="text-xs bg-cyan-100 text-cyan-800 px-2 py-0.5 rounded-full">{d}</span>
              ))
      ...
      {date && (
        <div className="mt-4 border-t pt-3">
          ...
          <p className="text-[11px] text-slate-500 mt-2">Times shown in UTC.</p>
        </div>
      )}
    </div>
  );
