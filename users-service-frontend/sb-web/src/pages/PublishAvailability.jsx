...
export default function PublishAvailability() {
  ...
  const canGenerate = useMemo(() => {
    ...
    return minutesBetween(start, end) > 0 && Number(slotLen) > 0;
  }, [mentorId, date, start, end, slotLen]);

  function generateSlots() {
    setGenError(''); setPreview([]); setResult(null); setApiErr('');
    if (!canGenerate) { setGenError('Fill Mentor ID, Date, Start/End, positive slot length.'); return; }
    const total = minutesBetween(start, end);
    const len = Number(slotLen);
    const gapM = Number(gap) || 0;
    if (len > total) { setGenError('Slot length > total range.'); return; }

    const slots = [];
    let cur = start;
    while (minutesBetween(cur, end) >= len) {
      const slotEndLocal = addMinutes(cur, len);
      slots.push({ start: localDateTimeToIsoUtc(date, cur), end: localDateTimeToIsoUtc(date, slotEndLocal) });
      cur = addMinutes(slotEndLocal, gapM);
    }
    if (slots.length === 0) { setGenError('No slots generated; adjust inputs.'); return; }
    setPreview(slots);
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white border rounded-xl p-5 shadow-sm space-y-4 max-w-3xl">
        ...
        <div className="grid md:grid-cols-4 gap-4">
          <label className="block">
            ...
            <input type="time" value={start} onChange={e=>setStart(e.target.value)}
                   className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-cyan-500" />
          </label>
          <label className="block">
          ...
          <TextInput label="Gap (min)" value={gap} onChange={setGap} />
        </div>

        <div className="flex gap-3">
          <button onClick={generateSlots} className="bg-slate-700 text-white px-4 py-2 rounded hover:bg-slate-800 transition text-sm">Generate</button>
          <button onClick={submit} disabled={preview.length===0 || saving}
        ...
        {result && <div className="text-green-700 text-sm">Created: <b>{result.created}</b> slots</div>}
      </div>
      ...
      <div className="bg-white border rounded-xl p-5 shadow-sm max-w-3xl">
        ...
        {preview.length === 0 ? (
          ...
          <ul className="text-sm list-disc pl-5 space-y-1">
            {preview.map((s, i) => (
              <li key={i} className="text-slate-700"><span className="font-mono">{s.start}</span> → <span className="font-mono">{s.end}</span> (UTC)</li>
            ))}
          </ul>
        )}
        <p className="text-xs text-slate-500 mt-2">Times are saved as UTC.</p>
      </div>
    </div>
  );
}
